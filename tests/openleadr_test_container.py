...
class OpenLeadrVtnTestContainer:
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            oauth_valid_audiences (str): The valid audiences for the OAuth token, comma seperated list of valid audiences.
            oauth_key_type (str, optional): The type of OAuth key. Defaults to "RSA".
            openleadr_rs_image (str, optional): The image to use for the VTN. Defaults to "ghcr.io/openleadr/openleadr-rs:latest".
        ...
        self._postgres = PostgresContainer(
            ...
            dbname=postgres_db
        ).with_network(self._network).with_network_aliases("vtndb")
        
        # Initialize VTN container with the static environment variables.
        self._vtn = DockerContainer(openleadr_rs_image, **kwargs) \
    ...
    def start(self) -> "OpenLeadrVtnTestContainer":
        ...
        vtn_db_url = self._postgres.get_connection_url(driver=None) \
    ...
    def _wait_for_ready(self) -> None:
        ...
        wait_for_logs(self._vtn, "listening on", timeout=30)
    ...
    def __enter__(self) -> "OpenLeadrVtnTestContainer":
